...
/*
  ...
  - No "use client": Vite SPA, not Next's app router.
  - The render loop stops when the element scrolls off screen. The snippet runs
    requestAnimationFrame plus a full-screen fragment shader forever, on every
    page that mounts it — that is a GPU burning for a background nobody is
    looking at.
  - Under prefers-reduced-motion it draws one frame and stops. A continuously
    churning gradient is exactly the kind of motion that setting exists to stop.
  - `style` was cast through `any` to satisfy a borrowed type; it is just
    CSSProperties.
  - webgl-error-boundary was imported but not supplied; it is written alongside.
*/
...
export function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    void main() {
      ...
      /* Only run while the element is actually on screen. A fragment shader
         over a full-width band is real GPU work, and none of it is worth doing
         for a background that has scrolled away. */
      let running = false;
      const start = () => {
        if (running || reduced) return;
        running = true;
        // resume from where the clock left off rather than jumping
