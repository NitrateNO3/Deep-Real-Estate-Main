...
/* Notes on this integration (differences from the source snippet):
   - The "use client" directive is dropped; that is a Next.js App Router marker
     and only produces a bundler warning in Vite.
   - The inner <div>s are <span>s. <button>'s content model is phrasing content,
     so a <div> inside it is invalid HTML. Absolutely-positioned spans render
     identically.
   - --path is recomputed on resize, not just on mount. It is derived from the
     button's pixel size, so it went stale whenever the button reflowed (font
     loading, responsive text, container resize) and the light drifted off edge.
   - Added an optional `href`, which renders an <a>. This replaces a link in the
     welcome section, and a <button> that navigates is wrong for both
...
export type StarButtonProps = StarButtonOwnProps &
  Omit<React.HTMLAttributes<HTMLElement>, keyof StarButtonOwnProps | 'color'>;
...
export function StarButton({
  ...
  return (
    <button
      type="button"
      ref={pathRef as React.Ref<HTMLButtonElement>}
      style={style}
      className={classes}
      {...props}
    >
